...
function renderOverview() {
  ...
  document.getElementById('stat-grid').innerHTML = `
    ...
    <div class="stat-card"><div class="stat-lbl">Gesamt-Km</div><div class="stat-val">${(activities.reduce((s,a)=>s+a.distance,0)/1000).toFixed(1)}k</div><div class="stat-unit">km gesamt</div></div>
...
function actRow(a) {
  ...
  return `<div class="act-row">
    <div class="act-left"><div class="act-name">${a.name}</div><div class="act-date">${fmtDate(a.date)}</div></div>
    <div class="act-right">
      <div class="act-stat"><div class="act-stat-val">${a.distance.toFixed(2)}</div><div class="act-stat-lbl">km</div></div>
      <div class="act-stat"><div class="act-stat-val"><span class="pace-badge ${pc}">${fmtPace(a.pace)}</span></div><div class="act-stat-lbl">/km</div></div>
      <div class="act-stat"><div class="act-stat-val">${fmtTime(a.movingTime)}</div><div class="act-stat-lbl">Zeit</div></div>
      ${a.avgHR>0?`<div class="act-stat"><div class="act-stat-val">${a.avgHR.toFixed(0)}</div><div class="act-stat-lbl">HF</div></div>`:''}
    </div>
  </div>`;
}
...
function renderCharts() {
  ...
  if (hasHR) {
    ...
    card.innerHTML = `
      <div class="zone-bar">${[1,2,3,4,5].map((z,i)=>`<div class="zone-seg" style="width:${(zones[z]/total*100).toFixed(1)}%;background:${colors[i]}"></div>`).join('')}</div>
      <div class="zone-legend">${[1,2,3,4,5].map((z,i)=>`<div class="zone-item"><div class="zone-dot" style="background:${colors[i]}"></div>${names[i]}: ${(zones[z]/total*100).toFixed(0)}%</div>`).join('')}</div>
    `;
  } else {
...
function renderGoalProgress() {
  ...
  document.getElementById('goal-progress-card').innerHTML = `
    <div class="gp-row">
      <div class="gp-block"><div class="gp-val">${best?fmtTime(best):'—'}</div><div class="gp-lbl">Aktuelle Bestzeit</div></div>
      <div class="gp-block" style="text-align:right"><div class="gp-val" style="color:var(--orange)">${fmtTime(planTarget)}</div><div class="gp-lbl">Dein Ziel</div></div>
    </div>
    <div class="gp-bar"><div class="gp-fill" style="width:${pct.toFixed(1)}%"></div></div>
    <div class="gp-pct">${pct.toFixed(0)}% des Ziels erreicht</div>
    <div class="gp-gap">${gapText}</div>
  `;
}
...
function renderPlanWeek() {
  ...
  document.getElementById('plan-sessions').innerHTML = sessions.map((s,i) => {
    ...
    return `<div class="session ${s.type==='rest'?'rest':''} ${isToday?'today':''} ${done?'done':''}">
      <div class="s-day">
        <div class="s-daynum">${day.getDate()}</div>
        <div class="s-dayname">${days[i]}</div>
        ${isToday?'<div class="s-today-dot"></div>':''}
      </div>
      <div>
        ...
        <div class="s-chips">
          ${s.dist?`<span class="chip chip-dist">${s.dist}</span>`:''}
          ${s.pace?`<span class="chip chip-pace">⚡ ${s.pace}</span>`:''}
          ${done?`<span class="chip chip-done">✓ ${dayRuns[0].distance.toFixed(1)} km</span>`:''}
        </div>
      </div>
    </div>`;
  }).join('');
...
function renderPRs() {
  ...
  document.getElementById('pr-grid').innerHTML = prs.map(p=>`
    <div class="pr-card">
      ...
      <div class="pr-time">${fmtTime(p.time)}</div>
      <div class="pr-date">${p.date?fmtDate(p.date):''}</div>
    </div>
  `).join('');
