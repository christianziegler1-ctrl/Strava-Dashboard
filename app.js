...
function saveToken(data) {
  localStorage.setItem('strava_token', JSON.stringify({ access_token: data.access_token, refresh_token: data.refresh_token, expires_at: data.expires_at, athlete: data.athlete }));
}
...
async function loadData() {
  ...
  try {
    ...
    if (cached?.activities && Object.keys(cached.activities).length > 0) {
      ...
      initApp();
      // Background sync: only new activities
      setSpinText('Prüfe auf neue Läufe…');
      const lastSync = cached.lastSync || 0;
      const newActs  = await fetchNewActivities(token, lastSync);
      if (newActs.length > 0) {
        const updates = {};
...
function parseAct(a) {
  ...
  return { id:a.id, name:a.name, date:new Date(a.start_date_local), distance:dist, movingTime:a.moving_time, elevation:a.total_elevation_gain||0, avgHR:a.average_heartrate||0, maxHR:a.max_heartrate||0, pace:dist>0?a.moving_time/dist:0, avgSpeed:a.average_speed };
}
...
function actRow(a) {
  ...
  return `<div class="act-row"><div class="act-left"><div class="act-name">${a.name}</div><div class="act-date">${fmtDate(a.date)}</div></div><div class="act-right"><div class="act-stat"><div class="act-stat-val">${a.distance.toFixed(2)}</div><div class="act-stat-lbl">km</div></div><div class="act-stat"><div class="act-stat-val"><span class="pace-badge ${pc}">${fmtPace(a.pace)}</span></div><div class="act-stat-lbl">/km</div></div><div class="act-stat"><div class="act-stat-val">${fmtTime(a.movingTime)}</div><div class="act-stat-lbl">Zeit</div></div>${a.avgHR>0?`<div class="act-stat"><div class="act-stat-val">${a.avgHR.toFixed(0)}</div><div class="act-stat-lbl">HF</div></div>`:''}</div></div>`;
}
...
function selectDist(km,el){planDist=km;document.querySelectorAll('.dist-btn').forEach(b=>b.classList.remove('active'));el.classList.add('active');const d={5:[0,20,0],10:[0,40,0],21.1:[1,40,0],42.2:[3,30,0]};const[h,m,s]=d[km]||[0,30,0];document.getElementById('p-hours').value=h||'';document.getElementById('p-min').value=m;document.getElementById('p-sec').value=s||'';updateGoBtn();}
function updateGoBtn(){const h=parseInt(document.getElementById('p-hours').value)||0;const m=parseInt(document.getElementById('p-min').value)||0;const s=parseInt(document.getElementById('p-sec').value)||0;document.getElementById('go-btn').disabled=!(planDist&&(h*3600+m*60+s)>0);}
function buildPlan(){const h=parseInt(document.getElementById('p-hours').value)||0;const m=parseInt(document.getElementById('p-min').value)||0;const s=parseInt(document.getElementById('p-sec').value)||0;planTarget=h*3600+m*60+s;weekOffset=0;const label=planDist===21.1?'Halbmarathon':planDist===42.2?'Marathon':`${planDist}K`;document.getElementById('plan-dist-label').textContent=label;document.getElementById('plan-goal-sub').textContent=`Ziel: ${fmtTime(planTarget)}`;renderGoalProgress();renderPlanWeek();renderThisWeekRuns();document.getElementById('plan-setup').style.display='none';document.getElementById('plan-view').style.display='block';}
function resetPlan(){document.getElementById('plan-setup').style.display='block';document.getElementById('plan-view').style.display='none';}
function renderGoalProgress(){const best=getBestTime(planDist);const pct=best?Math.min(100,(planTarget/best)*100):0;const gap=best?best-planTarget:null;const gapText=gap===null?'Noch keine Vergleichsaktivität — leg los!':gap<=0?`🎉 Ziel bereits erreicht! Bestzeit: ${fmtTime(best)}`:`Noch ${fmtTime(gap)} bis zum Ziel`;document.getElementById('goal-progress-card').innerHTML=`<div class="gp-row"><div class="gp-block"><div class="gp-val">${best?fmtTime(best):'—'}</div><div class="gp-lbl">Aktuelle Bestzeit</div></div><div class="gp-block" style="text-align:right"><div class="gp-val" style="color:var(--orange)">${fmtTime(planTarget)}</div><div class="gp-lbl">Dein Ziel</div></div></div><div class="gp-bar"><div class="gp-fill" style="width:${pct.toFixed(1)}%"></div></div><div class="gp-pct">${pct.toFixed(0)}% des Ziels erreicht</div><div class="gp-gap">${gapText}</div>`;}
function renderThisWeekRuns(){const monday=getWeekMonday(weekOffset);const sunday=new Date(monday);sunday.setDate(monday.getDate()+6);sunday.setHours(23,59,59);const runs=activities.filter(a=>a.date>=monday&&a.date<=sunday);document.getElementById('plan-this-week').innerHTML=runs.length?runs.map(actRow).join(''):'<div class="empty">Noch keine Läufe diese Woche.</div>';}
function renderPlanWeek(){const monday=getWeekMonday(weekOffset);const today=new Date();today.setHours(0,0,0,0);const days=['Mo','Di','Mi','Do','Fr','Sa','So'];const sessions=generateWeek(planDist,planTarget,weekOffset);if(weekOffset===0)document.getElementById('wk-label').textContent='DIESE WOCHE';else if(weekOffset===1)document.getElementById('wk-label').textContent='NÄCHSTE WOCHE';else if(weekOffset===-1)document.getElementById('wk-label').textContent='LETZTE WOCHE';else{const e=new Date(monday);e.setDate(e.getDate()+6);document.getElementById('wk-label').textContent=`${fmtDateShort(monday)}–${fmtDateShort(e)}`;}document.getElementById('plan-sessions').innerHTML=sessions.map((s,i)=>{const day=new Date(monday);day.setDate(monday.getDate()+i);const isToday=day.getTime()===today.getTime();const dayRuns=activities.filter(a=>{const d=new Date(a.date);d.setHours(0,0,0,0);return d.getTime()===day.getTime();});const done=dayRuns.length>0&&s.type!=='rest';return`<div class="session ${s.type==='rest'?'rest':''} ${isToday?'today':''} ${done?'done':''}"><div class="s-day"><div class="s-daynum">${day.getDate()}</div><div class="s-dayname">${days[i]}</div>${isToday?'<div class="s-today-dot"></div>':''}</div><div><div class="s-type ${s.type}">${done?'✓ ':''}${s.typeLabel}</div><div class="s-title">${s.title}</div><div class="s-desc">${s.desc}</div><div class="s-chips">${s.dist?`<span class="chip chip-dist">${s.dist}</span>`:''}${s.pace?`<span class="chip chip-pace">⚡ ${s.pace}</span>`:''}${done?`<span class="chip chip-done">✓ ${dayRuns[0].distance.toFixed(1)} km</span>`:''}</div></div></div>`;}).join('');}
function changeWeek(d){weekOffset+=d;renderPlanWeek();renderThisWeekRuns();}
function generateWeek(dist,target,offset){const tp=target/dist;const easy=tp*1.20;const tempo=tp*1.05;const intv=tp*0.93;const lng=tp*1.15;const recent=activities.filter(a=>(Date.now()-a.date)/86400000<=28);const baseKm=Math.max(20,recent.reduce((s,a)=>s+a.distance,0)/4);const wKm=Math.min(baseKm*(1+offset*0.05),baseKm*1.3);const plans={5:[{type:'easy',typeLabel:'Locker',title:'Lockerer Dauerlauf',desc:'Ruhig einlaufen, Nasenatmung.',dist:`${(wKm*.20).toFixed(1)} km`,pace:`${fmtPace(easy)}/km`},{type:'interval',typeLabel:'Intervall',title:'10 × 400m',desc:'10 Min einlaufen · 10×400m schnell, 90 Sek. Pause · 10 Min auslaufen.',dist:`${(wKm*.20).toFixed(1)} km`,pace:`${fmtPace(intv)}/km`},{type:'rest',typeLabel:'Ruhetag',title:'Aktive Erholung',desc:'Kein Laufen. Stretching oder Spazieren.',dist:null,pace:null},{type:'tempo',typeLabel:'Tempo',title:'Tempodauerlauf',desc:`2 km ein · ${(wKm*.15).toFixed(1)} km Tempopace · 2 km aus.`,dist:`${(wKm*.25).toFixed(1)} km`,pace:`${fmtPace(tempo)}/km`},{type:'rest',typeLabel:'Ruhetag',title:'Erholung',desc:'Beine schonen.',dist:null,pace:null},{type:'long',typeLabel:'Langer Lauf',title:'Langer Lauf',desc:'Gleichmäßig. Letztes Drittel etwas schneller.',dist:`${(wKm*.30).toFixed(1)} km`,pace:`${fmtPace(lng)}/km`},{type:'rest',typeLabel:'Ruhetag',title:'Wochenabschluss',desc:'Regeneration.',dist:null,pace:null}],10:[{type:'easy',typeLabel:'Locker',title:'Regenerationslauf',desc:'Sehr locker, HF unter 140.',dist:`${(wKm*.15).toFixed(1)} km`,pace:`${fmtPace(easy)}/km`},{type:'interval',typeLabel:'Intervall',title:'6 × 800m',desc:'10 Min ein · 6×800m schnell, 400m Pause · 10 Min aus.',dist:`${(wKm*.20).toFixed(1)} km`,pace:`${fmtPace(intv)}/km`},{type:'easy',typeLabel:'Locker',title:'Mittellauf',desc:'Ruhig, Fokus Lauftechnik.',dist:`${(wKm*.15).toFixed(1)} km`,pace:`${fmtPace(easy)}/km`},{type:'tempo',typeLabel:'Tempo',title:'Schwellenlauf',desc:`2 km ein · ${(wKm*.20).toFixed(1)} km Schwellentempo · 2 km aus.`,dist:`${(wKm*.28).toFixed(1)} km`,pace:`${fmtPace(tempo)}/km`},{type:'rest',typeLabel:'Ruhetag',title:'Ruhe',desc:'Erholung ist auch Training.',dist:null,pace:null},{type:'long',typeLabel:'Langer Lauf',title:'Langer Lauf',desc:`${(wKm*.35).toFixed(1)} km gleichmäßig, letzte 3 km im 10K-Pace.`,dist:`${(wKm*.35).toFixed(1)} km`,pace:`${fmtPace(lng)}/km`},{type:'rest',typeLabel:'Ruhetag',title:'Wochenabschluss',desc:'Regeneration.',dist:null,pace:null}],21.1:[{type:'easy',typeLabel:'Locker',title:'Lockerer Dauerlauf',desc:'Sehr locker, HF unter 140.',dist:`${(wKm*.15).toFixed(1)} km`,pace:`${fmtPace(easy)}/km`},{type:'tempo',typeLabel:'HM-Pace',title:'Wettkampftempo',desc:`3 km ein · ${(wKm*.18).toFixed(1)} km HM-Tempo · 2 km aus.`,dist:`${(wKm*.25).toFixed(1)} km`,pace:`${fmtPace(tempo)}/km`},{type:'easy',typeLabel:'Locker',title:'Techniklauf',desc:'Locker, Schrittfrequenz ~180spm.',dist:`${(wKm*.12).toFixed(1)} km`,pace:`${fmtPace(easy)}/km`},{type:'interval',typeLabel:'Intervall',title:'4 × 2km Schwelle',desc:'10 Min ein · 4×2km Schwelle, 90 Sek. Pause · 10 Min aus.',dist:`${(wKm*.25).toFixed(1)} km`,pace:`${fmtPace(intv)}/km`},{type:'rest',typeLabel:'Ruhetag',title:'Ruhe',desc:'Beine schonen.',dist:null,pace:null},{type:'long',typeLabel:'Langer Lauf',title:'Langer Lauf',desc:`${(wKm*.35).toFixed(1)} km ruhig, letzten 5 km im HM-Pace.`,dist:`${(wKm*.35).toFixed(1)} km`,pace:`${fmtPace(lng)}/km`},{type:'rest',typeLabel:'Ruhetag',title:'Wochenabschluss',desc:'Früh schlafen, gut essen.',dist:null,pace:null}],42.2:[{type:'easy',typeLabel:'Locker',title:'Regenerationslauf',desc:'Sehr locker, HF unter 135.',dist:`${(wKm*.12).toFixed(1)} km`,pace:`${fmtPace(easy)}/km`},{type:'interval',typeLabel:'Intervall',title:'5 × 1km',desc:'10 Min ein · 5×1km HM-Tempo, 2 Min Pause · 10 Min aus.',dist:`${(wKm*.18).toFixed(1)} km`,pace:`${fmtPace(intv)}/km`},{type:'easy',typeLabel:'Locker',title:'Mittellauf',desc:'Locker und fließend.',dist:`${(wKm*.15).toFixed(1)} km`,pace:`${fmtPace(easy)}/km`},{type:'tempo',typeLabel:'MP-Lauf',title:'Marathon-Pace',desc:`3 km ein · ${(wKm*.20).toFixed(1)} km Marathon-Tempo · 3 km aus.`,dist:`${(wKm*.30).toFixed(1)} km`,pace:`${fmtPace(tempo)}/km`},{type:'rest',typeLabel:'Ruhetag',title:'Ruhe',desc:'Beine hochlegen.',dist:null,pace:null},{type:'long',typeLabel:'Langer Lauf',title:'Langer Lauf',desc:`${(wKm*.38).toFixed(1)} km gleichmäßig. Gel testen!`,dist:`${(wKm*.38).toFixed(1)} km`,pace:`${fmtPace(lng)}/km`},{type:'rest',typeLabel:'Ruhetag',title:'Wochenabschluss',desc:'Mental auftanken.',dist:null,pace:null}]};const key=dist<=5?5:dist<=10?10:dist<=21.1?21.1:42.2;return plans[key];}
